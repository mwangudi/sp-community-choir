...
import { massPlanForDate, orderedItems } from "@/lib/mass-plans";
import { cn } from "@/lib/utils";

export default function RepertoirePage() {
  const ctx = useMemo(() => liturgicalContext(new Date()), []);
  const suggested = useMemo(() => songsForSunday(ctx), [ctx]);
  const plan = useMemo(() => massPlanForDate(ctx.date), [ctx]);
  ...
  const filtered = useMemo(
    ...
    [query, season, massPart, language],
  );
        ...
        <div className="container">
          ...
          {filtered.length === 0 ? (
            <p className="mt-8 text-muted-foreground">
              No songs match those filters. Try clearing one — or browse the{" "}
              <a
                href={DRIVE_DATABASE_URL}
                target="_blank"
                rel="noopener noreferrer"
